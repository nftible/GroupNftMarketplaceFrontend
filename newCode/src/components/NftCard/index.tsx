...
export const NftCard = ({nft}) => {
  ...
  if(!metadata){
    if(nft?.token_uri && nft?.token_uri.includes("http")){
      console.log("nft?.token_uri ", nft?.token_uri);
      (async () => {
        const fetchmetaData = await axios.get(nft?.token_uri);
        metadata = fetchmetaData?.data;
        if(fetchmetaData?.data?.image && fetchmetaData?.data?.image.includes("ipfs://")){
          const cid = fetchmetaData?.data?.image.split("ipfs://")[1];
          console.log("cid", cid)
          metadata.image = "https://cloudflare-ipfs.com/ipfs/"+cid
        }
      })();
    }
  }
