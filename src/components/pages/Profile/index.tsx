  ...
  return (
      <div className="flex max-w-screen-3xl mx-auto flex-col items-center py-2.5 px-2">
        {userData?.coverImage ? (
          <>
            <img
              src={`${process.env.s3_url}/${userData?.coverImage}?${uuid()}`}
              className="object-cover bg-gray-200 w-full h-64 rounded-2xl"
            />
          </>
        ) : (
          <div className="flex justify-center items-center w-full h-64 rounded-2xl bg-gray-200"></div>
        )}
        {userData?.profileImage ? (
          <img
            src={`${process.env.s3_url}/${userData?.profileImage}?${uuid()}`}
        ...
        <div className="flex justify-center items-center flex-col mt-20">
          ...
          <div
            className="flex justify-center items-center rounded-2xl bg-gray-100 h-5 w-28 cursor-pointer hover:bg-gray-200 mb-4"
            ...
            <p className="text-xs" style={{ color: "rgb(128, 128, 128)" }}>
              {isCopied
                ...
                : `${userData?.publicAddress.substring(
                    0,
                    6
                  )}...${userData?.publicAddress.substr(
                    userData?.publicAddress.length - 4
                  )}`}
            </p>
          </div>
          <p className="font-normal text-md mb-4">@{userData?.username}</p>
          {showMore ? (
            <p
        ...
        <Tab.Group>
          <Tab.List className="capitalize border-b-2 border-gray-200  flex w-full justify-evenly p-4 mt-7 px-0 space-x-2 bg-blue-900/20 ">
            <Tab
              className={({ selected }) =>
                selected
                  ? "text-black capitalize lg:text-sm md:text-base sm:text-sm text-xs font-sans sm:font-semibold font-bold"
                  : "capitalize lg:text-sm md:text-base sm:text-sm text-xs font-sans sm:font-semibold font-bold text-gray-400"
              }
            >
              on Sale <span>46</span>{" "}
            </Tab>
            <Tab
              className={({ selected }) =>
                selected
